...
return string-join(
for $type in sort(distinct-values($coll//t:TEI[t:teiHeader/t:revisionDesc/@status='cleared']//t:persName/@type), $ARABIC)
let $files := distinct-values($coll//t:TEI[t:teiHeader/t:revisionDesc/@status='cleared']//t:persName[@type=$type]/base-uri())
return " * '" || $type || ": '&#x200E;" ||
    string-join(
        for $f in $files 
