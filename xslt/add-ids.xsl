...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="t:div[t:head[t:date/@when-custom]][//t:revisionDesc[@status='cleared']]">
        <xsl:copy>
            <xsl:choose>
                <xsl:when test="matches(@xml:id, 'W[AO][0-9]{11}-[0-9]+')">
                    <xsl:copy-of select="@*"/>
                </xsl:when>
                <xsl:otherwise>
    ...
    <xsl:template match="t:placeName[not(matches(@xml:id,'^place-W[AO][0-9]{11}-[0-9]+_\d{2}$'))][//t:revisionDesc[@status='cleared']]">
    ...
    <xsl:function name="fn:div-id">
        ...
        <xsl:sequence>W{upper-case(substring($lang,1,1))}{substring($div/t:head[1]/t:date[@when-custom]/@when-custom, 1, 4)}{normalize-space(fn:pad($issue, 4))}{fn:pad(string(count($div/preceding-sibling::t:div[fn:convert(t:head/t:bibl/t:biblScope[@unit='issue']) = $issue]) + 1), 2)}-{count($div/preceding-sibling::t:div[t:head/t:date/@when-custom]) + 1}</xsl:sequence>
    </xsl:function>
