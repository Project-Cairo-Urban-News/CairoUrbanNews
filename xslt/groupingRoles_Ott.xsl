    ...
    <xsl:template match="/" name="xsl:initial-template">
        ...
        <TEI>
            <teiHeader>
                <fileDesc>
                    <titleStmt>
                        <title>Roles in the Dataset - "Urbanism News In The Official Egyptian Gazette (al-Waqāʾiʿ
                ...
                <body>
                    <castList>
                        <castItem>
                            ...
                            <xsl:for-each-group select="$articlesOta//persName[@role]" group-by="@role">
                                ...
                                <role xml:lang="ota" xml:id="{concat('role_', generate-id())}">
                                    ...
                                    <xsl:value-of
                                        ...
                                        <!-- Find and list the contents of date elements with when-custom -->
                                            ...
                                            <note>
                                                <xsl:for-each select="$articlesOta//div[.//persName[@role = current-grouping-key()]]/date[@when-custom]">
                                            ...
                                            <!-- Find the when-custom attribute in the head/date element within each div -->
                                                <xsl:for-each select="$articlesOta//div[.//persName[@role = current-grouping-key()]]/head/date[@when-custom]">
                                                <date when-custom="{@when-custom}">
                                                    <xsl:value-of select="."/>
                                                    <!-- Extract xml:id from the parent div and add it in an idno element -->
                                                    <xsl:variable name="parentDiv" select="ancestor::div[@xml:id][1]"/>
